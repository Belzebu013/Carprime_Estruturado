		...
		<div class="area_login">
			<form class="formulario" action="login.php" method="post">
				...
				<div class="mb-3">
					...
					<input type="password" class="form-control" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" name="senha">
				</div>
				<div>
			...
			<?php
				...
				if(isset($_POST['login'])){
					$user = $_POST['user'];
					$senha = $_POST['senha'];
					$login = $conn -> prepare('SELECT * FROM `acessos` WHERE `email_cad`= ? AND `senha_cad` = ?');
					$login -> bindParam(1, $user);
					$login -> bindParam(2, $senha);
					$login -> execute();
					if($login -> rowcount()==0)
						echo "<h2 class='invalid_user' id='erro'>Login ou senha invalida!</h2>";
					}else{
						//inicia sessao

						session_start();
						$row = $login -> fetch();
						$_SESSION['login'] = $row['id_cliente'];

						//redirecionamento para area principal
